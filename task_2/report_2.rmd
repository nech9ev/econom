...
Плацебо-тест "в пространстве" используется для проверки надежности, сопоставляя эффект вмешательства на тритмент-группу с потенциально похожими эффектами на некотные контрольные группы, которые фактически не подвергались лечению. Это позволяет оценить насколько наблюдаемый эффект уникален для тритмент-группы и не связан с другими факторами, влияющими на контрольные группы.
...
before <- sqrt(
  mean(
    (dataprep.out$Y1plot[1:7] - (dataprep.out$Y0plot[1:7] %*% synth.out$solution.w[1:7]))^2
  )
)
before
after <- sqrt(
  mean(
    (dataprep.out$Y1plot[8:18] - (dataprep.out$Y0plot[8:18] %*% synth.out$solution.w[8:18]))^2
  )
)
after
after / before
...
## Задание 5
Можно было бы увеличить количество предикторов в модели синтетического анализа, попробовать найти предикты которые не были учтены при построении модели,
но значимо влияют на неё